...
def main():
    ...
    comparison_table = all_metrics.pivot_table(
        index='date',
        columns='model',
        values=['brier', 'log_loss', 'mae']
    )
    comparison_table.to_csv('model_comparison.csv')

    models = all_metrics['model'].unique()
    dates = pd.to_datetime(all_metrics[all_metrics['model'] == models[0]]['date'])
    ...
    for i, model in enumerate(models):
        ...
        axes[0].plot(model_dates, model_data['brier'].values,
                    marker=markers[i % len(markers)], label=model, linewidth=2,
                    color=colors[i], markersize=8)
        axes[1].plot(model_dates, model_data['log_loss'].values,
                    marker=markers[i % len(markers)], label=model, linewidth=2,
                    color=colors[i], markersize=8)
        axes[2].plot(model_dates, model_data['mae'].values,
                    marker=markers[i % len(markers)], label=model, linewidth=2,
                    color=colors[i], markersize=8)
