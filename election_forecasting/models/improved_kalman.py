...
class ImprovedKalmanModel(ElectionForecastModel):
    ...
    def fit_and_forecast(self, state_polls, forecast_date, election_date, actual_margin):
        ...
        dates = (recent_polls['middate'] - recent_polls['middate'].min()).dt.days.values.astype(float)
        ...
        poll_mean, poll_var = self.kalman_filter_rts(dates, adjusted_margins, obs_variance, mu, sigma2)
        ...
        # Simulate forward
        final_margins = self.simulate_forward(poll_mean, P_current, mu, sigma2, days_to_election, N=2000)
