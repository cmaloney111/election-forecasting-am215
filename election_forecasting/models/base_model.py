...
from election_forecasting.utils.data_utils import load_polling_data, load_election_results, compute_metrics
...
class ElectionForecastModel(ABC):
    ...
    def run_forecast(self, forecast_dates=None, min_polls=10, verbose=False):
        ...
        if forecast_dates is None:
            forecast_dates = [pd.to_datetime(d) for d in
                            ['2016-10-01', '2016-10-15', '2016-11-01', '2016-11-07']]

        polls, actual_margin = self.load_data()
        election_date = pd.to_datetime('2016-11-08')

        states = [s for s in polls['state_code'].unique() if pd.notna(s) and s in actual_margin]
            ...
            for forecast_date in forecast_dates:
                ...
                try:
                    result = self.fit_and_forecast(
                        train_polls, forecast_date, election_date, actual_margin.get(state)
                    )
    ...
    def plot_state(self, state):
        ...
        ax.fill_between(forecast_dates,
                        ...
                        predicted_margins + 1.645 * margin_stds,
                        alpha=0.25, color='lightblue', label='90% confidence interval', zorder=1)

        # Plot raw polls
        ax.scatter(state_polls['middate'], state_polls['margin'],
                  alpha=0.5, s=40, label='Raw polls', color='gray', zorder=2, marker='o')

        # Plot model forecast line on top of everything
        ax.plot(forecast_dates, predicted_margins, 'b-o', linewidth=3,
                markersize=10, label=f'{self.name} forecast', zorder=4,
                markeredgecolor='white', markeredgewidth=1.5)

        # Add reference lines
        ax.axhline(0, color='k', linestyle='--', alpha=0.5, linewidth=1, zorder=0)
        if state in actual_margin:
            ax.axhline(actual_margin[state], color='red', linestyle='--',
                      linewidth=2, label='Actual result', zorder=4)
        ...
        ax.set_title(f'{state} - {self.name} Forecast Evolution', fontsize=13, fontweight='bold')
