...
def load_election_results():
    ...
    """
    results = pd.read_csv('data/election_results/mit_president_state_1976_2020.csv', sep='\t')
    results_2016 = results[results['year'] == 2016].copy()

    state_results = results_2016.groupby(['state_po', 'party_simplified']).agg({'candidatevotes': 'sum'}).reset_index()
    dem = state_results[state_results['party_simplified'] == 'DEMOCRAT'].set_index('state_po')['candidatevotes']
    rep = state_results[state_results['party_simplified'] == 'REPUBLICAN'].set_index('state_po')['candidatevotes']
...
def load_fundamentals():
    ...
    for state in results['state_po'].unique():
        ...
        for year, margins_dict in [(2012, margins_2012), (2008, margins_2008)]:
            ...
            year_grouped = year_results.groupby('party_simplified')['candidatevotes'].sum()
...
def get_state_list(polls, actual_results):
    ...
    Returns:
        list of state codes
    """
    states = [s for s in polls['state_code'].unique()
              if pd.notna(s) and s in actual_results]
    return states
